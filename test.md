...
> 第一～八步针对**本地新版**；**第九步**改在**正式服务器 47.76.226.134** 上执行。
> （该机已于 2026-09-13 更新到与本地同版本 `bf5370b`，并完成 HTTPS 上线、pm2 开机自启、nginx 缓存头加固。）
...
## 第九步：并发 / 压力测试（生产服务器 47.76.226.134）

> 承接第八步：同样的脚本与判定口径，改在**正式服务器**上跑，回答「约 1000 人同时写入『浏览摊位+1』能否扛住、要多久」。
> 环境：阿里云 **4 vCPU / 7.3GB 内存**，Ubuntu 22.04，Node **v22.23.2**。

### 9.1 隔离方式（生产不碰）
- 副本库：`cp database.db database.test.db`
- 临时后端：`DATABASE_PATH=database.test.db PORT=3001 HOST=127.0.0.1 node src/server.js`（生产 `campus-api` 在 3000，全程未受影响）
- 客户端：`BASE_URL=http://127.0.0.1:3001` + keep-alive 50 条（同第八步，模拟 `用户 → nginx 连接池 → Node`）
- 收尾核实：3001 已释放、副本库与临时文件已删、生产库 `booth_view_records` 仍 11 行（未被改动）、`https://clubfairmap.com/` 仍 200

### 9.2 结果：各档位全部通过

| 并发请求 | 成功 | 5xx | `database is locked` | 网络错误 | 查库行数 | 总耗时 | 每请求 |
|---|---|---|---|---|---|---|---|
| **1000** | **1000** | 0 | 0 | 0 | 1000 | **2534ms** | 2.53ms |
| 2000 | 2000 | 0 | 0 | 0 | 2000 | 4942ms | 2.47ms |
| 5000 | 5000 | 0 | 0 | 0 | 5000 | 12081ms | 2.42ms |
| 10000 | 10000 | 0 | 0 | 0 | 10000 | 24291ms | 2.43ms |

（组 1~3 亦 4/4 通过，与第八步一致，不重复列出。）

### 9.3 排队机制：请求「同时发出」，但排两道队

1. **客户端连接池**：keep-alive 50 条 → 同时只有 50 个在途，其余在客户端排队等连接。
2. **服务器（硬串行）**：Node 单线程 + `node:sqlite` 的 `DatabaseSync` **同步阻塞**——每个请求的事务（开连接→BEGIN→UPSERT→计数→COMMIT）执行期间整个进程冻结，**严格逐个处理，无任何并行**。

**证据**：每请求耗时**不随并发变化**（1000 并发 2.53ms / 10000 并发 2.43ms）。若服务器能并行，总耗时会低于线性增长；实测**近线性伸缩**，正是串行队列的特征。

### 9.4 关键结论
- **承载量 ≈ 400 次写入/秒**（1/2.5ms），这是同步 SQLite 串行处理的上限，**不随并发增大而提高**。
- **并发变大只增加排队深度与延迟，不产生错误**：10000 并发下最后一个请求等了约 24s，仍返回 200 ——「会变慢，不会崩」。
- 1000 并发「浏览摊位+1」**约 2.5 秒**处理完，**0 个 5xx、0 个锁、0 网络错误**。对比本地基线 4659ms，服务器更快。
- 真实活动比测试乐观：1000 人不会同一毫秒点击，且上报发生在**停留 3 秒后**，请求天然摊开在时间轴上。需警惕的是「**持续**每秒 >400 次写入」，而非「瞬时 1000 并发」。

### 9.5 发现的问题
- **服务器无问题**。
- **测试脚本夹具缺陷**（`backend/src/test/test-concurrency.js:61`）：`BOOTH_POOL` 写死 **20 个摊位**，而 `BOOTHS_PER_DEVICE = CONC_BROWSE / 100`。当 `CONC_BROWSE > 2000` 时每设备需 >20 个摊位，`BOOTH_POOL[b]` 为 `undefined`，这些请求全打到 `/booths/undefined/view`（同一行），使「查库行数」断言误报（5000/10000 时均报 =2100，即 100×(20+1)）。
  - 该误报**非服务器缺陷**：同一批请求全部返回 200、0 锁、0 超时。
  - 将摊位池放大到 1000 后重跑，四档全部 4/4 通过（上表即修正后数据）。
  - **建议修复**：`BOOTH_POOL` 长度改为 `Math.max(20, CONC_BROWSE / 100)`，否则该脚本只能可信地测到 2000 并发。

### 已知限制（本步）
- 压测客户端与后端**同机**（127.0.0.1，无网络延迟）——测的是纯服务端处理能力；但也与服务器抢同一份 CPU，真实外网端到端会更高。
- 副本库仅 77KB，数据量小；生产数据增长后每次事务成本会上升。
- 架构限制同第八步：Node 单线程 + 同步 SQLite，**串行处理**不变。
